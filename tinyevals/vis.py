...
def to_tok_prob_str(tok, prob, tokenizer):
    tok_str = tokenizer.decode(tok).replace(" ", "&nbsp;").replace("\n", r"\n")
    prob_str = f"{prob:.2%}"
    return f"{prob_str:>6} |{tok_str}|"

def token_to_html(token: Int64, tokenizer: PreTrainedTokenizer, bg_color: str=None, data: dict=None):
...
def vis_sample_prediction_probs(
    sample_tok: Int64[Tensor, "pos"], 
    correct_probs: Float32[Tensor, "next_pos"], 
    top_k_probs: topk, 
    ...
    for i in range(sample_tok.shape[0]):
        ...
        token_htmls.append(
            token_to_html(
                tok, tokenizer, bg_color=colors[i], data=data
